    ...
    static func menuItems(section: Int) -> Array<MenuItem> {
        ...
        if section == 0 {
            return [
                ...
                /*MenuItem(storyboardID: "SettingsViewController", name: "Settings", image: UIImage(named: "ic_side_settings")?.imageWithRenderingMode(.AlwaysTemplate), highlightedImage: nil, color: mainColor, action: .Navigation)*/MenuItem(storyboardID: nil, name: "Sign Out", image: nil, highlightedImage: nil, color: UIColor.redColor(), action: .Selector)]
        } else if section == 1 {
